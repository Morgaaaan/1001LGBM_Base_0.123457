...
def split_by_alert(
    ...
    # === 準備輸出 ===
    out_both = os.path.join(out_dir, "txn_from_to_both_in_alert.csv")
    out_from = os.path.join(out_dir, "txn_from_only_in_alert.csv")
    out_to   = os.path.join(out_dir, "txn_to_only_in_alert.csv")
    out_none = os.path.join(out_dir, "txn_neither_in_alert.csv")
